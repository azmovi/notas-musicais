...
def test_escala_cli_deve_conter_as_notas_na_resposta_de_fa(grau):
    result = runner.invoke(app, ['escala', 'F'])
    assert grau in result.stdout


@mark.parametrize('nota', ['C', 'E', 'G'])
def test_notas_cli_deve_conter_as_notas_na_resposta_de_fa(nota):
    result = runner.invoke(app, ['acorde'])
    assert nota in result.stdout


@mark.parametrize('grau', ['I', 'III', 'V'])
def test_acorde_cli_deve_conter_as_notas_na_resposta_de_fa(grau):
